 ...
 *
 * @returns {Promise<SoloClientSession[]>} - A promise that resolves to an array of client game sessions.
 */
async function getSessionsFromRedis(): Promise<SoloClientSession[]> {
  let cursor = 0
  const sessions: SoloClientSession[] = []
  ...
  do {
    ...
    if (sessionKeys.length > 0) {
      const data = (await redis.json.mget<SoloClientSession[][]>(sessionKeys, "$"))
