...
const PlayerBadge = ({
  player, hideVerified = false, showVerifiedTooltip = false,
  ...
  return (
    <Badge className={cn(playerBadgeVariants({ size }), className)}
      ...
      {...props}
    >
      <Hash className={cn("size-3.5 transition group-hover:opacity-95", {
        "size-3": size === "sm",
        "size-4": size === "lg"
      })}
        strokeWidth={2.5}
      ...
      <PlayerVerified className={cn("size-3.5 ml-0.5", {
        "size-3": size === "sm",
        "size-4": size === "lg",
